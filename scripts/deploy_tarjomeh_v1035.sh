...
BACKUP="$BACKUP_ROOT/v1035-$STAMP"
ROLLBACK="translate_tarjomeh:rollback-v1035-$STAMP"
CANDIDATE="translate_tarjomeh:candidate-v1035-$STAMP"
OVERLAY_DOCKERFILE=""
...
# A 9router update can leave its previous image consuming roughly 1 GB. The
# user has authorized removal of historical images, but never the running
# container or its exact image. Container references are checked before every
# deletion and the active 9router identity is verified again after deployment.
while IFS= read -r historical_nine; do
    [ -z "$historical_nine" ] && continue
    historical_id="$(docker image inspect -f '{{.Id}}' "$historical_nine")"
    [ "$historical_id" = "$NINE_IMAGE" ] && continue
    if docker ps -aq | xargs -r docker inspect -f '{{.Image}}' |
        grep -Fxq "$historical_id"; then
        continue
    fi
    docker image rm "$historical_id" >/dev/null 2>&1 || true
done < <(docker images decolua/9router --no-trunc --format '{{.ID}}' | sort -u)
...
# v10.35 is code-only. Build explicitly on the exact running image instead of
...
OVERLAY_DOCKERFILE="$(mktemp /tmp/tarjomeh-v1035-overlay.XXXXXX.Dockerfile)"

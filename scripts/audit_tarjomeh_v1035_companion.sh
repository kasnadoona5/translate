audit_tarjomeh_v1035_companion() {
    cd /opt/translate || {
...
# Confirm the running container, not merely Git, contains v10.35 behavior.
...
runtime_markers = {
    "runtime_release_contract": runtime_manifest.get("release") == "v10.35.0",
...
if not all(runtime_markers.values()):
    hard.append("running_container_is_not_complete_v1035")
...
json_path = Path(f"/app/jobs/uploads/{job_id}_v1035_companion_audit.json")
...
txt_path = Path(f"/app/jobs/uploads/{job_id}_v1035_companion_audit.txt")

lines = [
    "Tarjomeh v10.35 Independent Companion Audit",
    f"Job: {job_id}",
...
PY
    ...
    RESULT=$?
    if [ "$RESULT" -ne 0 ]; then
        echo "ERROR: v10.35 companion audit failed"
        return "$RESULT"
    fi
    ...
    find jobs/uploads -maxdepth 1 -type f \
      \( -name '*_v1035_companion_audit.txt' \
         -o -name '*_v1035_companion_audit.json' \) \
      -printf '%TY-%Tm-%Td %TH:%TM %10s %p\n' | sort | tail -4
    echo "Root console copy: /root/tarjomeh_v1035_companion_audit.txt"
    echo "Read-only v10.35 companion audit completed. Nothing was changed."
}

audit_tarjomeh_v1035_companion "${1:-LATEST}"
unset -f audit_tarjomeh_v1035_companion

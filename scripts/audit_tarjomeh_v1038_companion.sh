audit_tarjomeh_v1038_companion() {
    cd /opt/translate || {
...
source_obligation_fresh_events = [
    {"timestamp": event["timestamp"], "chunk_index": event["chunk_index"],
     **event["payload"]}
    for event in events
    if event["event_type"] == "source_obligation_fresh_generation"
]
source_obligation_exhausted_events = [
    {"timestamp": event["timestamp"], "chunk_index": event["chunk_index"],
     **event["payload"]}
    for event in events
    if event["event_type"] == "source_obligation_recovery_exhausted"
]
...
post_edit_attribution = [
    ...
    for event in active_events
    if event["event_type"] in {
        "newly_observed_unchanged_issue", "edit_introduced_issue",
...
unsafe_accepted_terminology = [
    ...
    and not provenance[source].get("context_deferred")
    and provenance[source].get("alignment_status") == "exact_local"
    and not is_reusable_terminology_mapping(source, target)
]
exact_final_quality_repairs = [
    {"chunk_index": event["chunk_index"], **event["payload"]}
    for event in active_events
    if event["event_type"] == "exact_final_quality_repair"
]
surplus_note_markers = []
for chunk in chunks:
    if chunk.get("status") not in {"completed", "needs_review"}:
        continue
    required_notes = collections.Counter(
        extract_note_markers(str(chunk.get("text", "")))
    )
    available_notes = available_note_markers(
        str(chunk.get("translation", "")), required_notes
    )
    surplus = list((available_notes - required_notes).elements())
    if surplus:
        surplus_note_markers.append({
...
style_dimension_violations = [
    {
        "index": index,
        "scores": dict(record.get("final_scores", {}) or {}),
    }
    for index, record in enumerate(style_sample_records)
    if isinstance(record, dict)
    and record.get("representative")
    and (
        any(
            name not in dict(record.get("final_scores", {}) or {})
            for name in ("accuracy", "fluency", "terminology", "register")
        )
        or any(
            float(dict(record.get("final_scores", {}) or {}).get(name, 0) or 0)
            < 9.0
            for name in ("accuracy", "fluency", "terminology", "register")
        )
    )
...
# Confirm the running container, not merely Git, contains v10.38 behavior.
...
runtime_markers = {
    "runtime_release_contract": runtime_manifest.get("release") == "v10.38.0",
    "runtime_behavior_contract": all(runtime_probes.values()),
    "focused_attachment_trial_stays_off": (
        runtime_manifest.get("capabilities", {}).get(
            "focused_attachment_trial_off_by_default"
        ) is True
        and "review_attachment(" not in pipeline_source
    ),
...
if not all(runtime_markers.values()):
    hard.append("running_container_is_not_complete_v1038")
...
if surplus_note_markers:
    hard.append("surplus_source_note_marker_in_finished_translation")
if style_dimension_violations:
    review.append("representative_style_record_below_dimension_floor")
...
summary_payload = {
    ...
    "pipeline": {
        ...
        "source_obligation_fresh_events": source_obligation_fresh_events,
        "source_obligation_exhausted_events": source_obligation_exhausted_events,
        ...
        "post_edit_attribution": post_edit_attribution,
        "exact_final_quality_repairs": exact_final_quality_repairs,
        "surplus_note_markers": surplus_note_markers,
    ...
    "memory": {
        ...
        "style_dimension_violations": style_dimension_violations,
        "unsafe_grounded_reliable_memory": unsafe_grounded_memory,
...
json_path = Path(f"/app/jobs/uploads/{job_id}_v1038_companion_audit.json")
...
txt_path = Path(f"/app/jobs/uploads/{job_id}_v1038_companion_audit.txt")

lines = [
    "Tarjomeh v10.38 Independent Companion Audit",
    f"Job: {job_id}",
    ...
    f"Fresh source-obligation translations: {len(source_obligation_fresh_events)}",
    f"Exhausted source-obligation recoveries: {len(source_obligation_exhausted_events)}",
    ...
    f"Exact final-quality repairs: {exact_final_quality_repairs or 'none'}",
    f"Surplus source note markers: {surplus_note_markers or 'none'}",
    "",
    "LLM",
...
lines.extend([
    ...
    f"Style dimension violations: {style_dimension_violations or 'none'}",
    ...
    f"Post-edit attribution events: {post_edit_attribution or 'none'}",
    f"Spaced optional plurals: "
    f"{sum(int(item.get('spaced_optional_plural_count', 0) or 0) for item in language_quality_reviews)}",
    f"Detached ezafe: "
    f"{sum(int(item.get('detached_ezafe_count', 0) or 0) for item in language_quality_reviews)}",
    f"Unbalanced dashes: "
    f"{sum(int(item.get('unbalanced_explanatory_dash_count', 0) or 0) for item in language_quality_reviews)}",
...
PY
    ...
    RESULT=${PIPESTATUS[0]}
    if [ "$RESULT" -ne 0 ]; then
        echo "ERROR: v10.38 companion audit failed"
        return "$RESULT"
    fi
    ...
    find jobs/uploads -maxdepth 1 -type f \
      \( -name '*_v1038_companion_audit.txt' \
         -o -name '*_v1038_companion_audit.json' \) \
      -printf '%TY-%Tm-%Td %TH:%TM %10s %p\n' | sort | tail -4
    echo "Root console copy: /root/tarjomeh_v1038_companion_audit.txt"
    echo "Read-only v10.38 companion audit completed. Nothing was changed."
}

audit_tarjomeh_v1038_companion "${1:-LATEST}"
unset -f audit_tarjomeh_v1038_companion

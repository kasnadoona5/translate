audit_tarjomeh_v1036_companion() {
    cd /opt/translate || {
...
# Confirm the running container, not merely Git, contains v10.36 behavior.
...
runtime_markers = {
    "runtime_release_contract": runtime_manifest.get("release") == "v10.36.0",
...
if not all(runtime_markers.values()):
    hard.append("running_container_is_not_complete_v1036")
...
json_path = Path(f"/app/jobs/uploads/{job_id}_v1036_companion_audit.json")
...
txt_path = Path(f"/app/jobs/uploads/{job_id}_v1036_companion_audit.txt")

lines = [
    "Tarjomeh v10.36 Independent Companion Audit",
    f"Job: {job_id}",
...
PY
    ...
    RESULT=${PIPESTATUS[0]}
    if [ "$RESULT" -ne 0 ]; then
        echo "ERROR: v10.36 companion audit failed"
        return "$RESULT"
    fi
    ...
    find jobs/uploads -maxdepth 1 -type f \
      \( -name '*_v1036_companion_audit.txt' \
         -o -name '*_v1036_companion_audit.json' \) \
      -printf '%TY-%Tm-%Td %TH:%TM %10s %p\n' | sort | tail -4
    echo "Root console copy: /root/tarjomeh_v1036_companion_audit.txt"
    echo "Read-only v10.36 companion audit completed. Nothing was changed."
}

audit_tarjomeh_v1036_companion "${1:-LATEST}"
unset -f audit_tarjomeh_v1036_companion

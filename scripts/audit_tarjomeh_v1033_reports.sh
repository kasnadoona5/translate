collect_tarjomeh_v1033_reports() {
    cd /opt/translate || {
...
json_path = upload_dir / f"{job_id}_v1033_memory_audit.json"
txt_path = upload_dir / f"{job_id}_v1033_memory_audit.txt"
json_path.write_text(
    json.dumps(bundle, ensure_ascii=False, indent=2), encoding="utf-8"
)

lines = [
    "Tarjomeh v10.33 Memory, Style, and QA Companion Report",
    f"Job: {job_id}",
...
PY
    ...
    RESULT=$?
    if [ "$RESULT" -ne 0 ]; then
        echo "ERROR: v10.33 report collection failed"
        return "$RESULT"
    fi
    ...
    find jobs/uploads -maxdepth 1 -type f \
      \( -name '*_qa_report.txt' -o -name '*_v1033_memory_audit.txt' \
         -o -name '*_v1033_memory_audit.json' \) \
      -printf '%TY-%Tm-%Td %TH:%TM %10s %p\n' | sort | tail -6
    echo "Read-only report collection completed. The job was not changed."
}

collect_tarjomeh_v1033_reports "${1:-LATEST}"
unset -f collect_tarjomeh_v1033_reports

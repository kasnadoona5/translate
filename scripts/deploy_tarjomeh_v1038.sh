...
BACKUP="$BACKUP_ROOT/v1038-$STAMP"
ROLLBACK="translate_tarjomeh:rollback-v1038-$STAMP"
CANDIDATE="translate_tarjomeh:candidate-v1038-$STAMP"
OVERLAY_DOCKERFILE=""
...
PY
    ...
    echo "STOPPED: could not verify active Tarjomeh jobs."
    exit 1
fi
...
while IFS= read -r stale_rollback; do
    ...
    rollback_id="$(docker image inspect -f '{{.Id}}' "$stale_rollback")"
    [ "$rollback_id" = "$OLD_IMAGE" ] && continue
    [ "$rollback_id" = "$NINE_IMAGE" ] && continue
    if docker ps -aq | xargs -r docker inspect -f '{{.Image}}' |
        grep -Fxq "$rollback_id"; then
        continue
    fi
...
# Failed deployments can leave candidate tags behind. Remove only candidates
# whose image is not used by any container and is not either active runtime.
while IFS= read -r stale_candidate; do
    [ -z "$stale_candidate" ] && continue
    candidate_id="$(docker image inspect -f '{{.Id}}' "$stale_candidate")"
    [ "$candidate_id" = "$OLD_IMAGE" ] && continue
    [ "$candidate_id" = "$NINE_IMAGE" ] && continue
    if docker ps -aq | xargs -r docker inspect -f '{{.Image}}' |
        grep -Fxq "$candidate_id"; then
        continue
    fi
    docker image rm "$stale_candidate" >/dev/null 2>&1 || true
done < <(docker images translate_tarjomeh --format '{{.Repository}}:{{.Tag}}' |
    grep '^translate_tarjomeh:candidate-' || true)
...
df -h /
if [ "$AVAILABLE_KB" -lt 350000 ]; then
    echo "STOPPED: less than 350 MB remains for the shared-layer build."
    echo "The running Tarjomeh image and 9router were not removed or changed."
    exit 1
fi

echo "========== BUILD SHARED-LAYER TARJOMEH CANDIDATE =========="
# v10.38 is code-only. Build explicitly on the exact running image instead of
...
OVERLAY_DOCKERFILE="$(mktemp /tmp/tarjomeh-v1038-overlay.XXXXXX.Dockerfile)"
...
checks = {
    ...
    "second_stage_transactional_replay": (
        "_recover_non_regressed_local_edits" in sources["pipeline"]
        and "partial_non_regressed_replay" in sources["pipeline"]
    ),
    "typed_dash_attachment_audit": (
        "_SPACED_EM_DASH_RE" in sources["pipeline"]
        and "persian_object_marker_detached_by_dash" in sources["pipeline"]
    ),
    "source_scoped_originals": (
        "source_applicable_originals" in sources["pipeline"]
        and "authorized_original_not_grounded_in_source_paragraph"
        in sources["term_notes"]
    ),
    "role_scoped_entity_memory": (
        "semantic_role" in sources["proper"]
        and "source_target_number_scope_mismatch" in sources["proper"]
    ),
    "normalized_english_summary_digits": (
        "_normalize_english_summary_digits" in sources["summary"]
    ),
    "long_llm_call_progress": "llm_call_in_progress" in sources["pipeline"],
    "research_role_evidence": (
        "semantic_role" in sources["research"]
        and "exact_evidence_quote" in sources["research"]
    ),
    ...
    "checkpoint_preview_atomic_publish": (
        "_publish_chapter_checkpoint" in sources["pipeline"]
        and "partial_path.replace(output_path)" in sources["pipeline"]
        and "persist_job_output=False" in sources["pipeline"]
        and "complete_chapter_checkpoint" in sources["pipeline"]
    ),
    "durable_chapter_checkpoint_recovery": (
        "chapter_checkpoint=chapter_checkpoint" in sources["pipeline"]
        and "_recover_pending_chapter_checkpoint" in sources["pipeline"]
        and "save_pending_chapter_checkpoint" in sources["database"]
        and "fail_chapter_checkpoint" in sources["database"]
        and "pending_export" in sources["database"]
    ),
    ...
    "boundary_safe_local_salvage": (
        "_replace_local_span_with_boundary_guard" in sources["pipeline"]
        and "boundary_deduplication" in sources["pipeline"]
    ),
    "grounded_objective_language_repair": (
        "_grounded_objective_language_issues" in sources["pipeline"]
        and "resolved_objective_issue_ids" in sources["pipeline"]
    ),
    "canonical_citation_memory_text": (
        normalize_citation_house_style_text(
            "(see Jessop 1990, 2002)"
        )[0] == "(ر.ک. Jessop 1990, 2002)"
        and "canonical_target_hash" in sources["pipeline"]
        and "canonical_target_hash" in sources["manager"]
    ),
    "first_person_dedication_style_filter": (
        "(?:I|we)\\s+dedicate" in sources["manager"]
    ),
    "academic_ezafe_examples": (
        sources["prompts"].count("هٔ") >= 2
        and "never add blanket diacritics" in sources["prompts"]
    ),
    "canonical_paragraph_identity": (
        "_canonical_chunk_paragraph_identity" in sources["pipeline"]
        and "canonical_chunk_paragraphs_v1" in sources["database"]
    ),
    "reconstructed_identity_review_only": (
        "_record_reconstructed_paragraph_identity_review"
        in sources["pipeline"]
        and "paragraph_identity_reconstructed" in sources["pipeline"]
    ),
    "verified_source_coverage": (
        "checked_source_segment_ids" in sources["critique"]
        and "coverage_gap_without_grounded_issue" in sources["critique"]
    ),
    "final_identifier_admission": (
        "final_source_artifact_reconciliation" in sources["pipeline"]
        and "Final source-identifier admission failed" in sources["pipeline"]
    ),
    "paragraph_scoped_refiner_salvage": (
        "_paragraph_scoped_span_start" in sources["pipeline"]
    ),
    "post_rollback_final_evidence": (
        "final_critique_rep = post_critique" in sources["pipeline"]
        and "post_rollback_final_validation" in sources["pipeline"]
    ),
    "objective_candidate_ranking": (
        "_grounded_objective_language_issues(critique)" in sources["pipeline"]
        and "final_candidate_selection" in sources["pipeline"]
    ),
    "contextual_morphology_quarantine": (
        "contextual_productive_suffix" in sources["proper"]
    ),
    "representative_only_established_style": (
        "active_records" in sources["manager"]
        and "unresolved_final_quality_issue" in sources["manager"]
    ),
    "candidate_bound_final_quality": (
        "candidate_target_hash" in sources["pipeline"]
        and "issue_fingerprint" in sources["pipeline"]
        and "final_retained_candidate_validation" in sources["pipeline"]
    ),
    "atomic_candidate_selection": (
        "candidate_selection is not None" in sources["database"]
        and "candidate_selection=(" in sources["pipeline"]
    ),
    "paragraph_scoped_readability": (
        "_readability_review_text" in sources["pipeline"]
    ),
    "lexical_scope_memory_quarantine": (
        "target_omits_source_lexical_member" in sources["proper"]
        and "foreign_expression_requires_review" in sources["proper"]
    ),
    "source_aligned_note_marker_relocation": (
        "relocated_aligned_sentence_terminal_note_marker"
        in sources["integrity"]
    ),
    "lexically_safe_critique_rebind": (
        "_critique_survives_canonicalization" in sources["pipeline"]
        and "lexically_identical_canonical_rebind" in sources["pipeline"]
    ),
    "identity_before_final_quality": (
        "Paragraph identity is part of the exact candidate" in sources["pipeline"]
        and sources["pipeline"].find("_canonical_chunk_paragraph_identity(chunk, translation)")
        < sources["pipeline"].find("canonical_candidate_hash = _candidate_text_hash")
    ),
    "atomic_final_quality_checkpoint": (
        "final_quality_admission is not None" in sources["database"]
        and "final_quality_admission=(" in sources["pipeline"]
    ),
    "resumable_source_obligation_repair": (
        "source_obligation_recovery_v1" in sources["pipeline"]
        and "source_obligation_recovery_reused" in sources["pipeline"]
        and "source_obligation_resolution=" in sources["pipeline"]
        and "source_obligation_resolution is not None" in sources["database"]
    ),
    "exact_final_quality_repair": (
        "exact_final_quality_repair" in sources["pipeline"]
        and "_minimal_unique_local_edit" in sources["pipeline"]
    ),
    "aligned_reviewed_memory": (
        "alignment_status" in sources["proper"]
        and "_reconcile_committed_terminology" in sources["pipeline"]
    ),
    "source_bounded_notes": "_PLAIN_TRAILING_NOTE_RE" in sources["integrity"],
    "dimension_gated_style": "final_scores" in sources["manager"],
}
print(checks)
assert all(checks.values()), checks
print("IMAGE_V1038_CONFIRMED")
'

echo "========== RECREATE TARJOMEH ONLY =========="
REPLACED=1
docker rm -f "$CONTAINER" >/dev/null 2>&1 || true
docker image tag "$CANDIDATE" translate_tarjomeh:latest
...
checks = {
    ...
    "second_stage_transactional_replay": (
        "_recover_non_regressed_local_edits" in p
        and "partial_non_regressed_replay" in p
    ),
    "typed_dash_attachment_audit": (
        "_SPACED_EM_DASH_RE" in p
        and "persian_object_marker_detached_by_dash" in p
    ),
    "source_scoped_originals": (
        "source_applicable_originals" in p
        and "authorized_original_not_grounded_in_source_paragraph" in o
    ),
    "role_scoped_entity_memory": (
        "semantic_role" in n
        and "source_target_number_scope_mismatch" in n
    ),
    "normalized_english_summary_digits": (
        "_normalize_english_summary_digits" in s
    ),
    "long_llm_call_progress": "llm_call_in_progress" in p,
    "research_role_evidence": (
        "semantic_role" in rsearch and "exact_evidence_quote" in rsearch
    ),
    ...
    "checkpoint_preview_atomic_publish": (
        "_publish_chapter_checkpoint" in p
        and "partial_path.replace(output_path)" in p
        and "persist_job_output=False" in p
        and "complete_chapter_checkpoint" in p
    ),
    "durable_chapter_checkpoint_recovery": (
        "chapter_checkpoint=chapter_checkpoint" in p
        and "_recover_pending_chapter_checkpoint" in p
        and "save_pending_chapter_checkpoint" in d
        and "fail_chapter_checkpoint" in d
        and "pending_export" in d
    ),
    "canonical_final_quality_authority": (
    ...
    "boundary_safe_local_salvage": (
        "_replace_local_span_with_boundary_guard" in p
        and "boundary_deduplication" in p
    ),
    "grounded_objective_language_repair": (
        "_grounded_objective_language_issues" in p
        and "resolved_objective_issue_ids" in p
    ),
    "canonical_citation_memory_text": (
        normalize_citation_house_style_text(
            "(see Jessop 1990, 2002)"
        )[0] == "(ر.ک. Jessop 1990, 2002)"
        and "canonical_target_hash" in p
        and "canonical_target_hash" in m
    ),
    "first_person_dedication_style_filter": "(?:I|we)\\s+dedicate" in m,
    "academic_ezafe_examples": (
        q.count("هٔ") >= 2 and "never add blanket diacritics" in q
    ),
    "canonical_paragraph_identity": (
        "_canonical_chunk_paragraph_identity" in p
        and "canonical_chunk_paragraphs_v1" in d
    ),
    "reconstructed_identity_review_only": (
        "_record_reconstructed_paragraph_identity_review" in p
        and "paragraph_identity_reconstructed" in p
    ),
    "verified_source_coverage": (
        "checked_source_segment_ids" in c
        and "coverage_gap_without_grounded_issue" in c
    ),
    "final_identifier_admission": (
        "final_source_artifact_reconciliation" in p
        and "Final source-identifier admission failed" in p
    ),
    "paragraph_scoped_refiner_salvage": (
        "_paragraph_scoped_span_start" in p
    ),
    "post_rollback_final_evidence": (
        "final_critique_rep = post_critique" in p
        and "post_rollback_final_validation" in p
    ),
    "objective_candidate_ranking": (
        "_grounded_objective_language_issues(critique)" in p
        and "final_candidate_selection" in p
    ),
    "contextual_morphology_quarantine": (
        "contextual_productive_suffix" in n
    ),
    "representative_only_established_style": (
        "active_records" in m
        and "unresolved_final_quality_issue" in m
    ),
    "candidate_bound_final_quality": (
        "candidate_target_hash" in p
        and "issue_fingerprint" in p
        and "final_retained_candidate_validation" in p
    ),
    "atomic_candidate_selection": (
        "candidate_selection is not None" in d
        and "candidate_selection=(" in p
    ),
    "paragraph_scoped_readability": "_readability_review_text" in p,
    "lexical_scope_memory_quarantine": (
        "target_omits_source_lexical_member" in n
        and "foreign_expression_requires_review" in n
    ),
    "source_aligned_note_marker_relocation": (
        "relocated_aligned_sentence_terminal_note_marker" in i
    ),
    "lexically_safe_critique_rebind": (
        "_critique_survives_canonicalization" in p
        and "lexically_identical_canonical_rebind" in p
    ),
    "identity_before_final_quality": (
        "Paragraph identity is part of the exact candidate" in p
        and p.find("_canonical_chunk_paragraph_identity(chunk, translation)")
        < p.find("canonical_candidate_hash = _candidate_text_hash")
    ),
    "atomic_final_quality_checkpoint": (
        "final_quality_admission is not None" in d
        and "final_quality_admission=(" in p
    ),
    "resumable_source_obligation_repair": (
        "source_obligation_recovery_v1" in p
        and "source_obligation_recovery_reused" in p
        and "source_obligation_resolution=" in p
        and "source_obligation_resolution is not None" in d
    ),
    "exact_final_quality_repair": (
        "exact_final_quality_repair" in p
        and "_minimal_unique_local_edit" in p
    ),
    "aligned_reviewed_memory": (
        "alignment_status" in n
        and "_reconcile_committed_terminology" in p
    ),
    "source_bounded_notes": "_PLAIN_TRAILING_NOTE_RE" in i,
    "dimension_gated_style": "final_scores" in m,
    "unchanged_issue_attribution": (
        "_proven_unchanged_issue" in p
        and "newly_observed_unchanged_issue" in p
        and "candidate_attribution_uncertain" in p
    ),
    "partial_readability_evidence": (
        "individually_valid_issue_count" in p
        and "readability_issues" in p
    ),
    "source_scoped_optional_plural": (
        "spaced_optional_plural_artifacts" in i
        and "source_optional_plural_spacing" in i
    ),
    "focused_reviewer_trial_only": (
        "review_attachment" in c
        and "review_attachment" not in p
    ),
}
print(checks)
assert all(checks.values()), checks
print("RUNNING_V1038_CONFIRMED")
'

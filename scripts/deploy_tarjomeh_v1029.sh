...
checks = {
    ...
    "checkpoint_preview_atomic_publish": (
        "_chapter_checkpoint_due" in sources["pipeline"]
        and "_record_chapter_checkpoint" in sources["pipeline"]
        and "output_path=partial_path" in sources["pipeline"]
        and "Chapter checkpoint preview export failed" in sources["pipeline"]
    ),
...
checks = {
    ...
    "checkpoint_preview_atomic_publish": (
        "_chapter_checkpoint_due" in p
        and "_record_chapter_checkpoint" in p
        and "output_path=partial_path" in p
        and "Chapter checkpoint preview export failed" in p
    ),
    "canonical_final_quality_authority": (

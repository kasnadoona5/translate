audit_tarjomeh_v1022_companion() {
    cd /opt/translate || {
    ...
    echo
    echo "========== V10.22 COMPANION AUDIT =========="
    docker exec -i translate_tarjomeh_1 env PYTHONIOENCODING=utf-8 \
      python - "$JOB" <<'PY' | tee /root/tarjomeh_v1022_companion_audit.txt
import collections
import json
import re
import sqlite3
...
# Confirm the running container, not merely Git, contains v10.22 behavior.
...
if not all(runtime_markers.values()):
    hard.append("running_container_is_not_complete_v1022")
if empty_finished:
...
json_path = Path(f"/app/jobs/uploads/{job_id}_v1022_companion_audit.json")
...
txt_path = Path(f"/app/jobs/uploads/{job_id}_v1022_companion_audit.txt")

lines = [
    "Tarjomeh v10.22 Independent Companion Audit",
    f"Job: {job_id}",
...
PY
    ...
    RESULT=$?
    if [ "$RESULT" -ne 0 ]; then
        echo "ERROR: v10.22 companion audit failed"
        return "$RESULT"
    fi
    ...
    find jobs/uploads -maxdepth 1 -type f \
      \( -name '*_v1022_companion_audit.txt' \
         -o -name '*_v1022_companion_audit.json' \) \
      -printf '%TY-%Tm-%Td %TH:%TM %10s %p\n' | sort | tail -4
    echo "Root console copy: /root/tarjomeh_v1022_companion_audit.txt"
    echo "Read-only v10.22 companion audit completed. Nothing was changed."
}

audit_tarjomeh_v1022_companion "${1:-${JOB:-LATEST}}"
unset -f audit_tarjomeh_v1022_companion

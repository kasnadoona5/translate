audit_tarjomeh_v1028_companion() {
    cd /opt/translate || {
...
final_canonical_chunks = {
    ...
    for event in active_events
    if event["event_type"] == "integrity_check_completed"
    and event["payload"].get("stage") == "final_canonical_admission"
}
...
canonical_event_by_chunk = {
    int(item["chunk_index"]): item for item in canonical_typography_events
}
...
# Confirm the running container, not merely Git, contains v10.28 behavior.
...
runtime_markers = {
    "runtime_release_contract": runtime_manifest.get("release") == "v10.28.0",
    ...
    "fluent_academic_contract": (
        ...
        and "natural academic Persian" in prompts_source
        and "never simplify" in prompts_source
    ),
    ...
    "checkpoint_preview_atomic_publish": (
        "_chapter_checkpoint_due" in pipeline_source
        and "_record_chapter_checkpoint" in pipeline_source
        and "output_path=partial_path" in pipeline_source
        and "Chapter checkpoint preview export failed" in pipeline_source
    ),
...
if not all(runtime_markers.values()):
    hard.append("running_container_is_not_complete_v1028")
...
json_path = Path(f"/app/jobs/uploads/{job_id}_v1028_companion_audit.json")
...
txt_path = Path(f"/app/jobs/uploads/{job_id}_v1028_companion_audit.txt")

lines = [
    "Tarjomeh v10.28 Independent Companion Audit",
    f"Job: {job_id}",
...
PY
    ...
    RESULT=$?
    if [ "$RESULT" -ne 0 ]; then
        echo "ERROR: v10.28 companion audit failed"
        return "$RESULT"
    fi
    ...
    find jobs/uploads -maxdepth 1 -type f \
      \( -name '*_v1028_companion_audit.txt' \
         -o -name '*_v1028_companion_audit.json' \) \
      -printf '%TY-%Tm-%Td %TH:%TM %10s %p\n' | sort | tail -4
    echo "Root console copy: /root/tarjomeh_v1028_companion_audit.txt"
    echo "Read-only v10.28 companion audit completed. Nothing was changed."
}

audit_tarjomeh_v1028_companion "${1:-LATEST}"
unset -f audit_tarjomeh_v1028_companion
